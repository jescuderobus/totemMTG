...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayComic(index) {
        ...
        comicDisplay.innerHTML = `<h3>${comic.title}</h3>
                                  <p>${comic.author}</p>
                                  <img id="comic-cover" src="${comic.cover2}" alt="${comic.title} cover">`;
        comicStatus.textContent = `${index + 1}/${comicsData.length} - ${comic.category}`;
    ...
    function displayFavorite(index) {
        ...
        favComicDisplay.innerHTML = `<h3>${comic.title}</h3>
                                     <p>${comic.author}</p>
                                     <img  id="comic-cover" src="${comic.cover2}" alt="${comic.title} cover">`;
        favComicStatus.textContent = `${index + 1}/${favoritesData.length} - ${comic.category}`;
    ...
    [prevComicBtn, nextComicBtn].forEach(btn => {
        btn.addEventListener('click', function() {
            if (btn === prevComicBtn) {
                currentComicIndex = (currentComicIndex - 1 + comicsData.length) % comicsData.length;
            } else {
    ...
    [prevFavComicBtn, nextFavComicBtn].forEach(btn => {
        btn.addEventListener('click', function() {
            if (btn === prevFavComicBtn) {
                currentFavoriteIndex = (currentFavoriteIndex - 1 + favoritesData.length) % favoritesData.length;
            } else {
                currentFavoriteIndex = (currentFavoriteIndex + 1) % favoritesData.length;
            }
            displayFavorite(currentFavoriteIndex);
        });
    });
